...
```bash
gh issue list --repo TheTatu13/peviitor-scraper-py --state open
```
...
After any code change that touches `config/company.json` or reads from it:
...
- `ai/COMPANY_MODEL.md` — schema must match `config/company.json`
- `ai/JOB_MODEL.md` — schema must match actual job documents
- `docs/README.md` — must describe the actual docs/ contents
- `docs/index.html` — branding must say `ANTIBIOTICE SA`, not the old template
...
- Remove tracked files that are gitignored (e.g., stray `test-report.html`)
- Delete empty directories that shouldn't exist (e.g., `tmp/`)
- Remove old path references in comments and docstrings
